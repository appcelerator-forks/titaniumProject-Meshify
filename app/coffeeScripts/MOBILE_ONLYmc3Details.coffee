...
$.MAScrollView.addEventListener "click", (e) ->
  try
    if e.source.titleValue.split(',')[0] == "1"
      name = (deviceName + "." + e.source.titleValue.split(',')[1])
      value = e.source.titleValue.split(',')[2]
      set.setButton(args.channels[name].channelId, args.channels[name].techName, name, args.macaddress, value, refresh)
      #ref.refNode(args, args.macaddress, args.node.NodeId, $.MAScrollView.getCurrentPage())
    else if e.source.titleValue.split(',')[0] == "2"
      gotoMode()
    else if e.source.titleValue.split(',')[0] == "3"
...
$.MAScrollView.addEventListener "touchstart", (e) ->
  try
    if e.source.titleValue.split(',')[0] > 0
...
$.MAScrollView.addEventListener "touchend", (e) ->
  try
    if e.source.titleValue.split(',')[0] > 0
      Ti.API.info e.source.titleValue
...
if args.channels[(deviceName + ".signal")].value
  args.channels[(deviceName + ".signal")].value = "-" + args.channels[(deviceName + ".signal")].value + " db"
...
try
  Ti.API.info 'args.person.CompanyObj.mobileLogoUrl: ' + args.person.CompanyObj.mobileLogoUrl
  $.logo.image = args.person.CompanyObj.mobileLogoUrl#"https://s3.amazonaws.com/LynxPRD-Production/companies/aaa%2Bmisting%2Bsystems/logos/mobilelogo.png?_=635488994377418642"#args.person.CompanyObj.mobileLogoUrl
catch e
  Ti.API.info e
#Ti.API.info args
